...
PHPAPI const char *php_version(void) { return PHP_VERSION; }

PHPAPI unsigned int php_version_id(void) { return PHP_VERSION_ID; }
...
#ifdef LOG_AUTH
  if (zend_string_equals_literal(facility, "LOG_AUTH") ||
      zend_string_equals_literal(facility, "auth") ||
...
#endif
#ifdef LOG_AUTHPRIV
  if (zend_string_equals_literal(facility, "LOG_AUTHPRIV") ||
      zend_string_equals_literal(facility, "authpriv")) {
    PG(syslog_facility) = LOG_AUTHPRIV;
    return SUCCESS;
  }
#endif
#ifdef LOG_CRON
  if (zend_string_equals_literal(facility, "LOG_CRON") ||
      zend_string_equals_literal(facility, "cron")) {
    PG(syslog_facility) = LOG_CRON;
    return SUCCESS;
  }
#endif
#ifdef LOG_DAEMON
  if (zend_string_equals_literal(facility, "LOG_DAEMON") ||
      zend_string_equals_literal(facility, "daemon")) {
    PG(syslog_facility) = LOG_DAEMON;
    return SUCCESS;
  }
#endif
#ifdef LOG_FTP
  if (zend_string_equals_literal(facility, "LOG_FTP") ||
      zend_string_equals_literal(facility, "ftp")) {
    PG(syslog_facility) = LOG_FTP;
    return SUCCESS;
  }
#endif
#ifdef LOG_KERN
  if (zend_string_equals_literal(facility, "LOG_KERN") ||
      zend_string_equals_literal(facility, "kern")) {
    PG(syslog_facility) = LOG_KERN;
    return SUCCESS;
  }
#endif
#ifdef LOG_LPR
  if (zend_string_equals_literal(facility, "LOG_LPR") ||
      zend_string_equals_literal(facility, "lpr")) {
    PG(syslog_facility) = LOG_LPR;
    return SUCCESS;
  }
#endif
#ifdef LOG_MAIL
  if (zend_string_equals_literal(facility, "LOG_MAIL") ||
      zend_string_equals_literal(facility, "mail")) {
    PG(syslog_facility) = LOG_MAIL;
    return SUCCESS;
  }
#endif
#ifdef LOG_INTERNAL_MARK
  if (zend_string_equals_literal(facility, "LOG_INTERNAL_MARK") ||
      zend_string_equals_literal(facility, "mark")) {
    PG(syslog_facility) = LOG_INTERNAL_MARK;
    return SUCCESS;
  }
#endif
#ifdef LOG_NEWS
  if (zend_string_equals_literal(facility, "LOG_NEWS") ||
      zend_string_equals_literal(facility, "news")) {
    PG(syslog_facility) = LOG_NEWS;
    return SUCCESS;
  }
#endif
#ifdef LOG_SYSLOG
  if (zend_string_equals_literal(facility, "LOG_SYSLOG") ||
      zend_string_equals_literal(facility, "syslog")) {
    PG(syslog_facility) = LOG_SYSLOG;
    return SUCCESS;
  }
#endif
#ifdef LOG_USER
  if (zend_string_equals(facility, ZSTR_KNOWN(ZEND_STR_USER)) ||
      zend_string_equals_literal(facility, "LOG_USER")) {
    PG(syslog_facility) = LOG_USER;
    return SUCCESS;
  }
#endif
#ifdef LOG_UUCP
  if (zend_string_equals_literal(facility, "LOG_UUCP") ||
      zend_string_equals_literal(facility, "uucp")) {
    PG(syslog_facility) = LOG_UUCP;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL0
  if (zend_string_equals_literal(facility, "LOG_LOCAL0") ||
      zend_string_equals_literal(facility, "local0")) {
    PG(syslog_facility) = LOG_LOCAL0;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL1
  if (zend_string_equals_literal(facility, "LOG_LOCAL1") ||
      zend_string_equals_literal(facility, "local1")) {
    PG(syslog_facility) = LOG_LOCAL1;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL2
  if (zend_string_equals_literal(facility, "LOG_LOCAL2") ||
      zend_string_equals_literal(facility, "local2")) {
    PG(syslog_facility) = LOG_LOCAL2;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL3
  if (zend_string_equals_literal(facility, "LOG_LOCAL3") ||
      zend_string_equals_literal(facility, "local3")) {
    PG(syslog_facility) = LOG_LOCAL3;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL4
  if (zend_string_equals_literal(facility, "LOG_LOCAL4") ||
      zend_string_equals_literal(facility, "local4")) {
    PG(syslog_facility) = LOG_LOCAL4;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL5
  if (zend_string_equals_literal(facility, "LOG_LOCAL5") ||
      zend_string_equals_literal(facility, "local5")) {
    PG(syslog_facility) = LOG_LOCAL5;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL6
  if (zend_string_equals_literal(facility, "LOG_LOCAL6") ||
      zend_string_equals_literal(facility, "local6")) {
    PG(syslog_facility) = LOG_LOCAL6;
    return SUCCESS;
  }
#endif
#ifdef LOG_LOCAL7
  if (zend_string_equals_literal(facility, "LOG_LOCAL7") ||
      zend_string_equals_literal(facility, "local7")) {
...
static PHP_INI_MH(OnChangeMemoryLimit) {
  ...
  if (zend_set_memory_limit(value) == FAILURE) {
    /* When the memory limit is reset to the original level during deactivation,
     * we may be using more memory than the original limit while shutdown is
     * still in progress. Ignore a failure for now, and set the memory limit
     * when the memory manager has been shut down and the minimal amount of
     * memory is used. */
    if (stage != ZEND_INI_STAGE_DEACTIVATE) {
      zend_error(E_WARNING,
                 "Failed to set memory limit to %zd bytes (Current memory "
                 "usage is %zd bytes)",
                 value, zend_memory_usage(true));
      return FAILURE;
    }
  }
  ...
  if (sapi_module.executable_location) {
    ...
    if (!strchr(sapi_module.executable_location, '/')) {
      ...
      if ((envpath = getenv("PATH")) != NULL) {
        ...
        while (search_dir) {
          snprintf(search_path, MAXPATHLEN, "%s/%s", search_dir,
                   sapi_module.executable_location);
          if (VCWD_REALPATH(search_path, binary_location) &&
    ...
    } else if (!VCWD_REALPATH(sapi_module.executable_location,
                              binary_location) ||
               VCWD_ACCESS(binary_location, X_OK)) {
      pefree(binary_location, 1);
      binary_location = NULL;
...
static uint8_t php_get_display_errors_mode(zend_string *value) {
  ...
  if (mode && mode != PHP_DISPLAY_ERRORS_STDOUT &&
...
static PHP_INI_DISP(display_errors_mode) {
  ...
  cgi_or_cli =
      (!strcmp(sapi_module.name, "cli") || !strcmp(sapi_module.name, "cgi") ||
...
STD_PHP_INI_ENTRY_EX("display_errors", "1", PHP_INI_ALL, OnUpdateDisplayErrors,
                     display_errors, php_core_globals, core_globals,
                     display_errors_mode)
STD_PHP_INI_BOOLEAN("display_startup_errors", "1", PHP_INI_ALL, OnUpdateBool,
...
STD_PHP_INI_BOOLEAN("expose_php", "1", PHP_INI_SYSTEM, OnUpdateBool, expose_php,
                    php_core_globals, core_globals)
STD_PHP_INI_ENTRY("docref_root", "", PHP_INI_ALL, OnUpdateString, docref_root,
                  php_core_globals, core_globals)
STD_PHP_INI_ENTRY("docref_ext", "", PHP_INI_ALL, OnUpdateString, docref_ext,
                  php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("html_errors", "1", PHP_INI_ALL, OnUpdateBool, html_errors,
                    php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("xmlrpc_errors", "0", PHP_INI_SYSTEM, OnUpdateBool,
                    xmlrpc_errors, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("xmlrpc_error_number", "0", PHP_INI_ALL, OnUpdateLong,
                  xmlrpc_error_number, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("max_input_time", "-1", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateLong, max_input_time, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("ignore_user_abort", "0", PHP_INI_ALL, OnUpdateBool,
                    ignore_user_abort, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("implicit_flush", "0", PHP_INI_ALL, OnUpdateBool,
                    implicit_flush, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("log_errors", "0", PHP_INI_ALL, OnUpdateBool, log_errors,
                    php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("ignore_repeated_errors", "0", PHP_INI_ALL, OnUpdateBool,
                    ignore_repeated_errors, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("ignore_repeated_source", "0", PHP_INI_ALL, OnUpdateBool,
                    ignore_repeated_source, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("report_memleaks", "1", PHP_INI_ALL, OnUpdateBool,
                    report_memleaks, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("report_zend_debug", "0", PHP_INI_ALL, OnUpdateBool,
                    report_zend_debug, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("output_buffering", "0", PHP_INI_PERDIR | PHP_INI_SYSTEM,
                  OnUpdateLong, output_buffering, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("output_handler", NULL, PHP_INI_PERDIR | PHP_INI_SYSTEM,
                  OnUpdateString, output_handler, php_core_globals,
                  core_globals)
STD_PHP_INI_BOOLEAN("register_argc_argv", "1", PHP_INI_PERDIR | PHP_INI_SYSTEM,
                    OnUpdateBool, register_argc_argv, php_core_globals,
                    core_globals)
STD_PHP_INI_BOOLEAN("auto_globals_jit", "1", PHP_INI_PERDIR | PHP_INI_SYSTEM,
                    OnUpdateBool, auto_globals_jit, php_core_globals,
                    core_globals)
...
STD_PHP_INI_ENTRY("unserialize_callback_func", NULL, PHP_INI_ALL,
                  OnUpdateString, unserialize_callback_func, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("serialize_precision", "-1", PHP_INI_ALL,
                  OnSetSerializePrecision, serialize_precision,
                  php_core_globals, core_globals)
STD_PHP_INI_ENTRY("arg_separator.output", "&", PHP_INI_ALL,
                  OnUpdateStringUnempty, arg_separator.output, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("arg_separator.input", "&", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateStringUnempty, arg_separator.input, php_core_globals,
                  core_globals)

STD_PHP_INI_ENTRY("auto_append_file", NULL, PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateString, auto_append_file, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("auto_prepend_file", NULL, PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateString, auto_prepend_file, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("doc_root", NULL, PHP_INI_SYSTEM, OnUpdateStringUnempty,
                  doc_root, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("default_charset", PHP_DEFAULT_CHARSET, PHP_INI_ALL,
                  OnUpdateDefaultCharset, default_charset, sapi_globals_struct,
                  sapi_globals)
STD_PHP_INI_ENTRY("default_mimetype", SAPI_DEFAULT_MIMETYPE, PHP_INI_ALL,
                  OnUpdateDefaultMimeTye, default_mimetype, sapi_globals_struct,
                  sapi_globals)
STD_PHP_INI_ENTRY("internal_encoding", NULL, PHP_INI_ALL,
                  OnUpdateInternalEncoding, internal_encoding, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("input_encoding", NULL, PHP_INI_ALL, OnUpdateInputEncoding,
                  input_encoding, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("output_encoding", NULL, PHP_INI_ALL, OnUpdateOutputEncoding,
                  output_encoding, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("error_log", NULL, PHP_INI_ALL, OnUpdateErrorLog, error_log,
                  php_core_globals, core_globals)
STD_PHP_INI_ENTRY("error_log_mode", "0644", PHP_INI_ALL, OnUpdateLong,
                  error_log_mode, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("extension_dir", PHP_EXTENSION_DIR, PHP_INI_SYSTEM,
                  OnUpdateStringUnempty, extension_dir, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("sys_temp_dir", NULL, PHP_INI_SYSTEM, OnUpdateStringUnempty,
                  sys_temp_dir, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("include_path", PHP_INCLUDE_PATH, PHP_INI_ALL,
                  OnUpdateStringUnempty, include_path, php_core_globals,
                  core_globals)
PHP_INI_ENTRY("max_execution_time", "30", PHP_INI_ALL, OnUpdateTimeout)
STD_PHP_INI_ENTRY("open_basedir", NULL, PHP_INI_ALL, OnUpdateBaseDir,
                  open_basedir, php_core_globals, core_globals)

STD_PHP_INI_BOOLEAN("file_uploads", "1", PHP_INI_SYSTEM, OnUpdateBool,
                    file_uploads, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("upload_max_filesize", "2M", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateLong, upload_max_filesize, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("post_max_size", "8M", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateLong, post_max_size, sapi_globals_struct,
                  sapi_globals)
STD_PHP_INI_ENTRY("upload_tmp_dir", NULL, PHP_INI_SYSTEM, OnUpdateStringUnempty,
                  upload_tmp_dir, php_core_globals, core_globals)
...
STD_PHP_INI_ENTRY("max_input_vars", "1000", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateLongGEZero, max_input_vars, php_core_globals,
                  core_globals)

STD_PHP_INI_ENTRY("user_dir", NULL, PHP_INI_SYSTEM, OnUpdateString, user_dir,
                  php_core_globals, core_globals)
STD_PHP_INI_ENTRY("variables_order", "EGPCS", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateStringUnempty, variables_order, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("request_order", NULL, PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateString, request_order, php_core_globals, core_globals)

STD_PHP_INI_ENTRY("error_append_string", NULL, PHP_INI_ALL, OnUpdateString,
                  error_append_string, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("error_prepend_string", NULL, PHP_INI_ALL, OnUpdateString,
                  error_prepend_string, php_core_globals, core_globals)

PHP_INI_ENTRY("SMTP", "localhost", PHP_INI_ALL, NULL)
PHP_INI_ENTRY("smtp_port", "25", PHP_INI_ALL, NULL)
STD_PHP_INI_BOOLEAN("mail.add_x_header", "0", PHP_INI_SYSTEM | PHP_INI_PERDIR,
                    OnUpdateBool, mail_x_header, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("mail.mixed_lf_and_crlf", "0",
                    PHP_INI_SYSTEM | PHP_INI_PERDIR, OnUpdateBool,
                    mail_mixed_lf_and_crlf, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("mail.log", NULL, PHP_INI_SYSTEM | PHP_INI_PERDIR,
                  OnUpdateMailLog, mail_log, php_core_globals, core_globals)
...
PHP_INI_ENTRY("max_multipart_body_parts", "-1", PHP_INI_SYSTEM | PHP_INI_PERDIR,
              NULL)

STD_PHP_INI_BOOLEAN("allow_url_fopen", "1", PHP_INI_SYSTEM, OnUpdateBool,
                    allow_url_fopen, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("allow_url_include", "0", PHP_INI_SYSTEM, OnUpdateBool,
                    allow_url_include, php_core_globals, core_globals)
STD_PHP_INI_BOOLEAN("enable_post_data_reading", "1",
                    PHP_INI_SYSTEM | PHP_INI_PERDIR, OnUpdateBool,
                    enable_post_data_reading, php_core_globals, core_globals)

STD_PHP_INI_ENTRY("realpath_cache_size", "4096K", PHP_INI_SYSTEM, OnUpdateLong,
...
STD_PHP_INI_ENTRY("user_ini.filename", ".user.ini", PHP_INI_SYSTEM,
                  OnUpdateString, user_ini_filename, php_core_globals,
                  core_globals)
STD_PHP_INI_ENTRY("user_ini.cache_ttl", "300", PHP_INI_SYSTEM, OnUpdateLong,
                  user_ini_cache_ttl, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("hard_timeout", "2", PHP_INI_SYSTEM, OnUpdateLong,
                  hard_timeout, zend_executor_globals, executor_globals)
#ifdef PHP_WIN32
STD_PHP_INI_BOOLEAN("windows.show_crt_warning", "0", PHP_INI_ALL, OnUpdateBool,
                    windows_show_crt_warning, php_core_globals, core_globals)
#endif
STD_PHP_INI_ENTRY("syslog.facility", "LOG_USER", PHP_INI_SYSTEM, OnSetFacility,
                  syslog_facility, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("syslog.ident", "php", PHP_INI_SYSTEM, OnUpdateString,
                  syslog_ident, php_core_globals, core_globals)
STD_PHP_INI_ENTRY("syslog.filter", "no-ctrl", PHP_INI_ALL, OnSetLogFilter,
                  syslog_filter, php_core_globals, core_globals)
PHP_INI_END()
/* }}} */
...
PHPAPI bool php_during_module_startup(void) { return module_startup; }
/* }}} */

/* {{{ php_during_module_shutdown */
PHPAPI bool php_during_module_shutdown(void) { return module_shutdown; }
/* }}} */

/* {{{ php_get_module_initialized */
PHPAPI bool php_get_module_initialized(void) { return module_initialized; }
/* }}} */
...
PHPAPI ZEND_COLD void php_log_err_with_severity(const char *log_message,
  ...
  if (PG(error_log) != NULL) {
    ...
    fd = VCWD_OPEN_MODE(PG(error_log), O_CREAT | O_APPEND | O_WRONLY,
                        error_log_mode);
    if (fd != -1) {
      char *tmp;
      size_t len;
      ...
      error_time_str = php_format_date("d-M-Y H:i:s e", 13, error_time, 1);
#endif
      len = spprintf(&tmp, 0, "[%s] %s%s", ZSTR_VAL(error_time_str),
                     log_message, PHP_EOL);
#ifdef PHP_WIN32
...
PHPAPI size_t php_write(void *buf, size_t size) { return PHPWRITE(buf, size); }
/* }}} */
...
static zend_string *escape_html(const char *buffer, size_t buffer_len) {
  ...
  if (!result || ZSTR_LEN(result) == 0) {
    ...
    result = php_escape_html_entities_ex(
        (const unsigned char *)buffer, buffer_len, 0,
        ENT_COMPAT | ENT_HTML_SUBSTITUTE_ERRORS,
        /* charset_hint */ NULL, /* double_encode */ 1, /* quiet */ 1);
  }
  return result;
}
...
PHPAPI ZEND_COLD void php_verror(const char *docref, const char *params,
  ...
  if (PG(html_errors)) {
    zend_string *replace_buffer =
        escape_html(ZSTR_VAL(buffer), ZSTR_LEN(buffer));
  ...
  } else if (EG(current_execute_data) && EG(current_execute_data)->func &&
  ...
  if (is_function) {
    origin_len = (int)spprintf(&origin, 0, "%s%s%s(%s)", class_name, space,
                               function, params);
  } else {
  ...
  if (docref && is_function && PG(html_errors) && strlen(PG(docref_root))) {
    ...
    if (PG(html_errors)) {
      message = zend_strpprintf_unchecked(0, "%s [<a href='%s%s%s'>%s</a>]: %S",
                                          origin, docref_root, docref,
                                          docref_target, docref, buffer);
    } else {
      message =
          zend_strpprintf_unchecked(0, "%s [%s%s%s]: %S", origin, docref_root,
...
/* {{{ php_error_docref */
/* Generate an error which links to docref or the php.net documentation if
 * docref is NULL */
...
PHPAPI ZEND_COLD void php_error_docref_unchecked(const char *docref, int type,
                                                 const char *format, ...) {
  php_error_docref_impl(docref, type, format);
}
/* }}} */
...
PHPAPI ZEND_COLD void php_win32_docref2_from_error(DWORD error,
                                                   ...
                                                   const char *param2) {
  char *buf = php_win32_error_to_msg(error);
  php_error_docref2(NULL, param1, param2, E_WARNING, "%s (code: %lu)", buf,
                    error);
  php_win32_error_msg_free(buf);
}
#endif
...
static void report_zend_debug_error_notify_cb(int type,
  ...
  if (PG(report_zend_debug)) {
    ...
    zend_output_debug_string(trigger_break, "%s(%" PRIu32 ") : %s",
...
static ZEND_COLD void php_error_cb(int orig_type, zend_string *error_filename,
  ...
  if (display && ((EG(error_reporting) & type) || (type & E_CORE)) &&
    ...
    if (PG(log_errors) ||
        (!module_initialized &&
         (!PG(display_startup_errors) || !PG(display_errors)))) {
      char *log_buffer;
#ifdef PHP_WIN32
      if (type == E_CORE_ERROR || type == E_CORE_WARNING) {
        syslog(LOG_ALERT, "PHP %s: %s (%s)", error_type_str, ZSTR_VAL(message),
               GetCommandLine());
      }
#endif
      spprintf(&log_buffer, 0, "PHP %s:  %s in %s on line %" PRIu32,
               error_type_str, ZSTR_VAL(message), ZSTR_VAL(error_filename),
               error_lineno);
    ...
    if (PG(display_errors) &&
      ...
      if (PG(xmlrpc_errors)) {
        php_printf("<?xml "
                   "version=\"1.0\"?><methodResponse><fault><value><struct><"
                   "member><name>faultCode</name><value><int>" ZEND_LONG_FMT
                   "</int></value></member><member><name>faultString</"
                   "name><value><string>%s:%s in %s on line %" PRIu32
                   "</string></value></member></struct></value></fault></"
                   "methodResponse>",
                   PG(xmlrpc_error_number), error_type_str, ZSTR_VAL(message),
                   ZSTR_VAL(error_filename), error_lineno);
      } else {
        ...
        if (PG(html_errors)) {
          if (type == E_ERROR || type == E_PARSE) {
            zend_string *buf =
                escape_html(ZSTR_VAL(message), ZSTR_LEN(message));
            php_printf(
                "%s<br />\n<b>%s</b>:  %s in <b>%s</b> on line <b>%" PRIu32
                "</b><br />\n%s",
                STR_PRINT(prepend_string), error_type_str, ZSTR_VAL(buf),
                ZSTR_VAL(error_filename), error_lineno,
            ...
            php_printf_unchecked(
                "%s<br />\n<b>%s</b>:  %S in <b>%s</b> on line <b>%" PRIu32
                "</b><br />\n%s",
                STR_PRINT(prepend_string), error_type_str, message,
                ZSTR_VAL(error_filename), error_lineno,
                STR_PRINT(append_string));
          }
        } else {
          ...
          if ((!strcmp(sapi_module.name, "cli") ||
            ...
            php_printf_unchecked("%s\n%s: %S in %s on line %" PRIu32 "\n%s",
                                 STR_PRINT(prepend_string), error_type_str,
                                 message, ZSTR_VAL(error_filename),
  ...
  case E_USER_ERROR:
    EG(exit_status) = 255;
    if (module_initialized) {
      if (!PG(display_errors) && !SG(headers_sent) &&
...
PHPAPI char *php_get_current_user(void) {
  ...
  if (!pstat) {
    ...
    SG(request_info).current_user =
        estrndup(pwd->pw_name, SG(request_info).current_user_length);
#if defined(ZTS) && defined(HAVE_GETPWUID_R) && defined(_SC_GETPW_R_SIZE_MAX)
    efree(pwbuf);
#endif
    return SG(request_info).current_user;
...
PHP_FUNCTION(set_time_limit) {
  ...
  new_timeout_strlen =
      zend_spprintf(&new_timeout_str, 0, ZEND_LONG_FMT, new_timeout);

  key = ZSTR_INIT_LITERAL("max_execution_time", 0);
  if (zend_alter_ini_entry_chars_ex(key, new_timeout_str, new_timeout_strlen,
...
static FILE *php_fopen_wrapper_for_zend(zend_string *filename,
                                        zend_string **opened_path) {
  *opened_path = filename;
  return php_stream_open_wrapper_as_file(ZSTR_VAL(filename), "rb",
...
{
  return php_stream_open_for_zend_ex(handle, USE_PATH | REPORT_ERRORS |
                                                 STREAM_OPEN_FOR_INCLUDE);
}
/* }}} */
                                               ...
                                               int mode) /* {{{ */
{
  ...
  stream =
      php_stream_open_wrapper((char *)ZSTR_VAL(filename), "rb",
                              mode | STREAM_OPEN_FOR_ZEND_STREAM, &opened_path);
  if (stream) {
...
{
  return php_resolve_path(ZSTR_VAL(filename), ZSTR_LEN(filename),
                          PG(include_path));
}
/* }}} */
...
static ZEND_COLD void php_message_handler_for_zend(zend_long message,
                                                   const void *data) {
  switch (message) {
  case ZMSG_FAILED_INCLUDE_FOPEN: {
    char *tmp = estrdup((char *)data);
    php_error_docref("function.include", E_WARNING,
                     "Failed opening '%s' for inclusion (include_path='%s')",
                     php_strip_url_passwd(tmp), STR_PRINT(PG(include_path)));
    efree(tmp);
    break;
  }
  case ZMSG_FAILED_REQUIRE_FOPEN: {
    char *tmp = estrdup((char *)data);
    zend_throw_error(NULL, "Failed opening required '%s' (include_path='%s')",
                     php_strip_url_passwd(tmp), STR_PRINT(PG(include_path)));
    efree(tmp);
    break;
  }
  case ZMSG_FAILED_HIGHLIGHT_FOPEN: {
    char *tmp = estrdup((char *)data);
    php_error_docref(NULL, E_WARNING, "Failed opening '%s' for highlighting",
  ...
  case ZMSG_MEMORY_LEAK_REPEATED:
    ...
    if (EG(error_reporting) & E_WARNING) {
      ...
      if (message == ZMSG_MEMORY_LEAK_DETECTED) {
        ...
        snprintf(memory_leak_buf, 512,
                 "%s(%" PRIu32 ") :  Freeing " ZEND_ADDR_FMT
                 " (%zu bytes), script=%s\n",
                 t->filename, t->lineno, (size_t)t->addr, t->size,
                 SAFE_FILENAME(SG(request_info).path_translated));
        if (t->orig_filename) {
          char relay_buf[512];

          snprintf(relay_buf, 512,
                   "%s(%" PRIu32 ") : Actual location (location was relayed)\n",
  ...
  case ZMSG_LOG_SCRIPT_NAME: {
    ...
    if (datetime_str) {
      datetime_str[strlen(datetime_str) - 1] =
          0; /* get rid of the trailing newline */
      snprintf(memory_leak_buf, sizeof(memory_leak_buf),
               "[%s]  Script:  '%s'\n", datetime_str,
               SAFE_FILENAME(SG(request_info).path_translated));
    } else {
      snprintf(memory_leak_buf, sizeof(memory_leak_buf),
...
#ifdef HAVE_DTRACE
  DTRACE_REQUEST_STARTUP(
      SAFE_FILENAME(SG(request_info).path_translated),
      SAFE_FILENAME(SG(request_info).request_uri),
      (char *)SAFE_FILENAME(SG(request_info).request_method));
#endif /* HAVE_DTRACE */
...
void php_request_shutdown(void *dummy) {
  ...
  /* EG(current_execute_data) points into nirvana and therefore cannot be safely
   * accessed inside zend_executor callback functions.
   */
  EG(current_execute_data) = NULL;

  php_deactivate_ticks();

  /* 0. Call any open observer end handlers that are still open after a
   * zend_bailout */
  ...
  /* 4. Reset max_execution_time (no longer executing php code after response
   * sent) */
  ...
  /* 6. Shutdown output layer (send the set HTTP headers, cleanup output
   * handlers, etc.) */
  ...
  zend_try { zend_post_deactivate_modules(); }
  zend_end_try();

  /* 12. SAPI related shutdown*/
  zend_try { sapi_deactivate_module(); }
  zend_end_try();
  /* free SAPI stuff */
  ...
  zend_try { php_shutdown_stream_hashes(); }
  zend_end_try();
  ...
  /* Reset memory limit, as the reset during INI_STAGE_DEACTIVATE may have
   * failed. At this point, no memory beyond a single chunk should be in use. */
  zend_set_memory_limit(PG(memory_limit));
...
void dummy_invalid_parameter_handler(const wchar_t *expression,
    ...
    if (PG(windows_show_crt_warning)) {
      ...
      if (function) {
        if (file) {
          len = _snprintf(
              buf, sizeof(buf) - 1,
              "Invalid parameter detected in CRT function '%ws' (%ws:%u)",
              function, file, line);
        } else {
...
zend_result php_module_startup(sapi_module_struct *sf,
...
#ifdef ZTS
  ts_allocate_fast_id(
      &core_globals_id, &core_globals_offset, sizeof(php_core_globals),
      (ts_allocate_ctor)core_globals_ctor, (ts_allocate_dtor)core_globals_dtor);
  fprintf(stderr, "TSID core_globals_id = %d\n", core_globals_id);
#ifdef PHP_WIN32
  ...
  fprintf(stderr, "TSID php_win32_core_globals_id = %d\n",
          php_win32_core_globals_id);
#endif
#else
  memset(&core_globals, 0, sizeof(core_globals));
  php_startup_ticks();
  ...
  le_index_ptr =
      zend_register_list_destructors_ex(NULL, NULL, "index pointer", 0);
  ...
  /* Until the current ini values was setup, the current cp is 65001.
          If the actual ini values are different, some stuff needs to be
     updated. It concerns at least main_cwd_state and there might be more. As
     we're still in the startup phase, lets use the chance and reinit the
     relevant item according to the current codepage. Still, if ini_set() is
     used later on, a more intelligent way to update such stuff is needed.
  ...
  if (additional_module &&
      (zend_register_internal_module(additional_module) == NULL)) {
    return FAILURE;
  }
  ...
  if (sapi_module.additional_functions) {
    if ((module = zend_hash_str_find_ptr(&module_registry, "standard",
                                         sizeof("standard") - 1)) != NULL) {
      EG(current_module) = module;
      zend_register_functions(NULL, sapi_module.additional_functions, NULL,
                              MODULE_PERSISTENT);
  ...
  if ((module = zend_hash_str_find_ptr(&module_registry, "core",
                                       sizeof("core") - 1)) != NULL) {
  ...
  /* NOTE: If you add anything here, remember to add it to
   * build/Makefile.global! */
  {
    struct {
      const long error_level;
      const char *phrase;
      const char *directives[18]; /* Remember to change this if the number of
                                     directives change */
    } directives[2] = {
        {E_DEPRECATED,
        ...
        {E_CORE_ERROR,
         ...
         {"allow_call_time_pass_reference", "asp_tags",
          "define_syslog_variables", "highlight.bg", "magic_quotes_gpc",
          "magic_quotes_runtime", "magic_quotes_sybase", "register_globals",
          "register_long_arrays", "safe_mode", "safe_mode_gid",
          "safe_mode_include_dir", "safe_mode_exec_dir",
          "safe_mode_allowed_env_vars", "safe_mode_protected_env_vars",
          "zend.ze1_compatibility_mode", "track_errors", NULL}}};
...
#define OLD_CWD_SIZE 4096
  ...
  zend_try {
    ...
    /* Only lookup the real file path and add it to the included_files list if
     * already opened otherwise it will get opened and added to the
     * included_files list in zend_execute_scripts
     */
    if (primary_file->filename &&
        !zend_string_equals_literal(primary_file->filename,
                                    "Standard input code") &&
        primary_file->opened_path == NULL &&
        primary_file->type != ZEND_HANDLE_FILENAME) {
      if (expand_filepath(ZSTR_VAL(primary_file->filename), realfile)) {
        primary_file->opened_path =
            zend_string_init(realfile, strlen(realfile), 0);
    ...
    if (prepend_file_p && result) {
      result =
          zend_execute_script(ZEND_REQUIRE, NULL, prepend_file_p) == SUCCESS;
    }
    if (result) {
      result =
          zend_execute_script(ZEND_REQUIRE, retval, primary_file) == SUCCESS;
    }
    if (append_file_p && result) {
      result =
          zend_execute_script(ZEND_REQUIRE, NULL, append_file_p) == SUCCESS;
    }
  }
  zend_catch { result = false; }
  zend_end_try();
...
PHPAPI int php_handle_auth_data(const char *auth) {
  ...
  if (auth && auth_len > 0 &&
      zend_binary_strncasecmp(auth, auth_len, "Basic ", sizeof("Basic ") - 1,
                              sizeof("Basic ") - 1) == 0) {
    char *pass;
    zend_string *user;
  ...
  if (ret == -1 && auth && auth_len > 0 &&
      zend_binary_strncasecmp(auth, auth_len, "Digest ", sizeof("Digest ") - 1,
                              sizeof("Digest ") - 1) == 0) {
...
PHPAPI void php_reserve_tsrm_memory(void) {
  size_t n = TSRM_ALIGNED_SIZE(sizeof(zend_compiler_globals)) +
...
/* {{{ php_tsrm_startup */
PHPAPI bool php_tsrm_startup(void) { return php_tsrm_startup_ex(1); }
/* }}} */
#endif
